...
- `node scripts/check-independent-review.mjs --head <40-character-head>`: offline mode that collects every standalone `independent-review head=<40-character-head> verdict=pass|fail by=<full lowercase UUID>` marker across piped reviews and comments. Pass-only evidence exits 0, missing/legacy evidence exits 1, malformed input exits 2, and any fail evidence exits 3.
