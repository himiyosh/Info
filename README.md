...
- `tests/quality/quality-spawn-timeout.test.mjs`: keep every nested spawner on that shared budget, reject hardcoded per-file timeouts and opaque `assert.ifError` spawn reporting, and verify the default still fits inside the `.github/workflows/quality-baseline.yml` job budget.
