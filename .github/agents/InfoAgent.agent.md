...
- Quality baseline must execute `npm run check:independent-review -- --repo <owner/name> --pr <number> --head <40-character current head SHA>` only for the current OPEN pull request after the assigned independent reviewer has posted or edited the marker; rerun the failed job against the unchanged head. The live guard fetches exactly that pull request plus bounded review and issue-comment snapshots, fails closed for missing/malformed markers, stale heads, same-head fails, invalid reviewer UUIDs, and unusable snapshots, and skips a valid closed snapshot so merged or closed pull requests are never retroactively failed.
