...
- Use ordinary code review or security review when the change warrants it, and resolve substantive findings through normal GitHub review comments.
- Pin the pull request's full head SHA immediately before merge and require the fetched `headRefOid` to match it exactly.
- Run the documented `gh pr view <N> --json state,isDraft,headRefOid,mergeable,mergeStateStatus,statusCheckRollup | node scripts/check-merge-gate.mjs --head <40-character current head SHA>` pipeline and treat every nonzero exit as blocked.
- The helper is dependency-free, offline, and snapshot-only. Exit zero means only that the supplied JSON reports OPEN, non-draft, the requested head, MERGEABLE/CLEAN state, at least one check, and successful terminal status for every reported check.
...
- Re-fetch the full pull-request snapshot and rerun the objective gate immediately before merge. Never merge from cached JSON or an earlier successful invocation.
