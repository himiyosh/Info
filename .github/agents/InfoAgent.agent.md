...
- Require one exact trimmed marker line outside Markdown fenced code blocks in a non-null review or comment body: after trimming leading and trailing whitespace, the complete line must exactly equal `independent-review head=<40-character current head SHA> verdict=pass by=<full lowercase UUID>` for clearance or the same line with `verdict=fail` for a blocker. Prose may appear before or after the marker on separate lines in the same body; the entire body need not contain only the marker. Negations, prohibitions, questions, list or table examples, inline code, fenced code, and lines with any same-line prefix, suffix, punctuation, or prose do not satisfy the gate. The `by` value must be the assigned independent reviewer's full lowercase session UUID and must differ from the coordinator and implementation child session IDs; absent, shortened, uppercase, braced, or otherwise invalid UUIDs do not satisfy the gate. Each active marker must express exactly one verdict. Short, stale, wrong-head, legacy verdict-less, and malformed exact lines do not satisfy the gate.
...
- If a posted verdict is wrong, edit the original comment marker to `RETRACTED-independent-review head=<40-character reviewed head SHA> verdict=<pass|fail> by=<full lowercase UUID>` and retain a retraction reason in that comment. Do not merely add an opposite verdict: an active fail marker still wins.
...
- Quality baseline no longer runs the review guard. Because `by=` is only checked for UUID shape, CI could never tell an independent verdict from a self-issued one, so the red check bought pressure to post a marker rather than assurance that a review happened. Run `npm run check:independent-review -- --repo <owner/name> --pr <number> --head <40-character current head SHA>` by hand against the current OPEN pull request whenever a review is being tracked. The live guard fetches exactly that pull request plus bounded review and issue-comment snapshots, fails closed for missing/malformed markers, stale heads, same-head fails, invalid reviewer UUIDs, and unusable snapshots, and skips a valid closed snapshot so merged or closed pull requests are never retroactively failed.
