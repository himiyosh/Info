...
- **次のすべてを満たす merge は統括の判断で実行してよいという常設承認を、世代交代のたびに後継へ明示的に引き継ぐ。** (1) 競合がなく draft でない、(2) 必須の自動検証がすべて成功、(3) 任意レビューを実施した場合は blocking な指摘が残っていない、(4) 本番反映、秘密情報、権限、課金、公開範囲の変更を含まない、(5) merge 直前に PR の head と checks を取り直している。
- 最後の再取得条件は省略しない。承認が必要なのは 4 番目の条件に該当する変更だけとする。判断を保留したまま次へ進むことも、放置と同じ停止として扱う。
