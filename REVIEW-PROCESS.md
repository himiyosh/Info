...
5. **`by=` には起動した側のセッション UUID を書く。**
   サブエージェントにセッション UUID は無い。`by=` は「この判定に責任を負うセッション」を
   指すものとして書く(ゲートは元々、実装者と別人であることを検査していない)。
   **その代わり 3 の逐語掲載が監査の実体になる**ので、省略してはいけない。
   自分が実装した PR に対しては、たとえサブエージェントが判定しても `by=` を書かない
   — その PR の保証人にはなれない。
...
**あなた(サブエージェント)は PR に投稿しない。** 判定結果を親へ返すだけでよい。
投稿は親が行う(あなたにはセッション UUID が無く、マーカーを作れないため)。

返す内容:

- `verdict`: `pass` か `fail`
- 根拠: 何をどう測って、何が出たか。**再現できる形で**書く
  (実行したコマンド、注入した負のコントロールと結果、測定値)
- 素通りした注入があれば必ず含める

親はこれを**逐語で**貼り、次の 1 行を、**前後の行から独立して**置く
(前後に説明文は可、同一行への付加は不可、コードフェンスの中は無効)。

```
independent-review head=<40桁のhead SHA> verdict=pass|fail by=<親のセッションUUID(小文字フル)>
```
