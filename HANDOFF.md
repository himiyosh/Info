...
### 独立レビューを CI ゲートから外した(2026-08-09、廃止済み)

この節は当時の判断記録であり、現在の運用手順ではない。オーナー判断で
`quality-baseline.yml` の強制ステップだけを先に削除し、手動インフラは一時的に
残していた。この中間状態は 2026-08-13 の完全廃止で終了した。

- **理由**: セッション識別子は書式しか検査できず、CI はレビューの独立性を
  保証できなかった。保証しないゲートが赤で止まると、「レビューする」より
  「通過用の証跡を作る」習慣を育てる構造だった。
- **当時の変更**: PR CI の強制ステップと専用 wiring 契約を削除し、
  品質ベースラインから到達しないことを回帰テストで固定した。
- **当時の検証**: 47ファイル・280テスト全通過(ファイル単位ループ、フレーク0)。

### 独立レビュー用インフラを完全廃止(2026-08-13)

全プロジェクトから廃止するというオーナー指示に従い、Info に残っていた
手動インフラも削除した。通常のコードレビューとセキュリティレビューは
必要に応じて GitHub の標準機能で行う。

- 専用パーサー、コマンド、テスト、依頼文、運用手順を削除した。
- `check-merge-gate.mjs` は OPEN、draft、head SHA、mergeability、
  merge state、reported checks だけを見る客観的な snapshot 検証に縮小した。
- 品質ベースラインと現行の運用面に専用ゲートが戻らない回帰契約を追加した。
- リポジトリ変数、`main` の branch protection、ruleset に専用設定は無かった。
